...
**Default Weight**:
The Weight a Test receives when no timing data exists for it. If the project has any measured samples elsewhere, file size is scaled into seconds via a coefficient (median measured-seconds-per-KB across measured files), floored at 0.01; with no measurements to estimate a coefficient from, it falls back to raw file size in KB, floored at 1.0. Either way, a file that cannot be read gets 1.0 flat. This ensures the scheduler can operate without a fully populated Timing file, and keeps mixed measured/unmeasured suites comparing Weights in one unit.
...
**Batch**:
The unit of dispatch under Dynamic scheduling: the set of Tests handed to a Worker in one test-runner invocation. A Batch drains half of a queue's remaining predicted Weight, floored at a per-run minimum, so early Batches amortize runner boot cost and tail Batches stay fine-grained for balance. The floor is `MIN_BATCH_WEIGHT` (30s) once the project is calibrated; on a pure cold start it is scale-free instead, targeting ~5 Batches per Worker regardless of suite size. See `docs/design/stealer-policy.md`.
_Avoid_: Chunk, slice
